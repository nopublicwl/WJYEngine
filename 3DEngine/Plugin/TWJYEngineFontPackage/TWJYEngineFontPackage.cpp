/*
 * TWJYEngineFontPackage.cpp
 *
 *  Created on: 2010-2-24
 *      Author: DevLinux
 */
#include "TWJYEngineFontPackage.h"
#include "EngineFontAPI.h"
	WJYLibraryHandle CreateLibraryWJY(HPWJYEngine hEngine,void_WJY * ptrEx){
		WJY_FontLibraryHeavy * ptrInface;
		WJY_FontLibraryFunTable    * ptrTable;
	...
	//	char_WJY pPath[WJY3DSystemFileNameMaxLength];
		...
		void_WJY * ptrConstCT;
		int iConstCTSize;
		if((hEngine == NULL_WJY) || (g_hLibraryHandle_Font != NULL_WJY))
			return NULL_WJY;
		...
		if((ptrSysEnv == NULL_WJY) || (ptrInfo == NULL_WJY) || (hECTHandle == NULL_WJY))
	 	...
	 	iInfaceSize = WJY_CompilerAllocSize(WJY_FontLibraryHeavy);
		ptrInface   = (WJY_FontLibraryHeavy *)WJYSysAPI_Mem_AllocateMemoryPool(iInfaceSize);
	...
	*/
	...
	void_WJY Destory_LibraryWJY(HPWJYEngine hEngine,WJYLibraryHandle hAndle,void_WJY * ptrEx){
		...
		ptrInface = (WJY_FontLibraryHeavy *)hAndle;
		if((hEngine == NULL_WJY) || (hAndle == NULL_WJY))
			return;
		g_hLibraryHandle_Font = NULL_WJY;
		ptrSysEnv = WJYSysAPI_Sys_GetOperatSystemEnv(hEngine);
		ptrInfo   = WJYSysAPI_Sys_GetConstSystemInfo(hEngine);
