...
HSoundMgrAndle Eg_CreateSoundManager(HResDataSystem hRDSystem){
	WJY_ResourceSoundManager * ptrMgr;
	intV_WJY iCount;
	ptrMgr = (WJY_ResourceSoundManager *)WJYSysAPI_Mem_TLSAllocateMemoryPool(WJY_CompilerAllocSize(WJY_ResourceSoundManager));
	WJYSysAPI_MemoryClearZero(ptrMgr,WJY_CompilerAllocSize(WJY_ResourceSoundManager));
	...
	for(iCount = 0;iCount<WJYRMgr_ConstID_RSoundMgrMapSlotMaxSize;iCount++){
		WJYSysAPI_Syn_InitializeRWLockWF(ptrMgr->List[iCount].lock);
	}
	return (HSoundMgrAndle)ptrMgr;
}
void_WJY Eg_DestorySoundManager(HSoundMgrAndle hAndle){
	intV_WJY iCount;
	intV_WJY iNCount;
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem *    ptrItem;
	WJY_ResourceSoundItem *    ptrNItem;
	ptrMgr = (WJY_ResourceSoundManager *)hAndle;
	if(ptrMgr == NULL_WJY)
		return;
	ptrMgr->iTkCount = 0;
	...
	for(iCount = 0;iCount < WJYRMgr_ConstID_RSoundMgrMapSlotMaxSize;iCount++){
		...
		while(ptrNItem!=NULL_WJY){
			...
			__native_Eg_PushReclaimQueue<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,ptrItem);
		}
	}

	while((ptrItem = __native_Eg_PopReclaimQueue<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr))!=NULL_WJY){
		if(ptrItem->ptrName){
...
WJY_ResourceSound * Eg_CreateSoundMgrObject(HSoundMgrAndle hAndle,const_WJY char_WJY * ptrKey,intV_WJY iKeySize){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem *    ptrItem;
	WJY_ResourceSoundItem *    ptrNextItem;
	uintV_WJY uHash;
	ptrMgr = (WJY_ResourceSoundManager *)hAndle;
	ptrItem = NULL_WJY;
	uHash = ptrMgr->hashfunc(ptrKey,iKeySize,WJYRMgr_ConstID_RSoundMgrMapSlotMaxSize);
	...
	ptrItem = (WJY_ResourceSoundItem *)WJYSysAPI_Mem_TLSAllocateMemoryPool(WJY_CompilerAllocSize(WJY_ResourceSoundItem));
	if(ptrItem == NULL_WJY)
		return NULL_WJY;
	WJYSysAPI_MemoryClearZero(ptrItem,WJY_CompilerAllocSize(WJY_ResourceSoundItem));
	...
	ptrItem->eType = WJYSys_Const_ResD_eWJYSoundMgr;
	ptrItem->iCount = 1;
	ptrItem->iRefCount = 0;
...
WJY_ResourceSound * Eg_OpenSoundMgrObject(HSoundMgrAndle hAndle,const_WJY char_WJY * ptrKey,intV_WJY iKeySize){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem *    ptrItem;
	WJY_ResourceSoundItem *    ptrNItem;
	WJY_ResourceSoundItem *    ptrRelaseItem;
	...
	if((ptrNItem != NULL_WJY) && (ptrNItem->eDynamicState != WJYSys_Const_RODynamic_eReclaim)){
		...
		if(iRefCount < 0){
			...
			if((eDynamicState != WJYSys_Const_RODynamic_eReclaim) && (WJYSysAPI_JudgeSetValueInt((volatile_WJY intV_WJY *)(&(ptrItem->eDynamicState)),
				...
				__native_Eg_RemoveMapItem<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,uHash,ptrItem);
				ptrRelaseItem = ptrItem;
			}
			if(ptrRelaseItem != NULL_WJY){
				__native_Eg_PushReclaimQueue<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,ptrRelaseItem);
			}
			return NULL_WJY;
		}
	}
	WJYSysAPI_Syn_ReleaseRWLockReadWF(ptrMgr->List[uHash].lock);
	return (WJY_ResourceSound *)ptrItem;
}
HWJY_Result Eg_ReleaseSoundMgrObject(WJY_ResourceSound * ptrItem){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem * ptrPItem;
	WJY_ResourceSoundItem * ptrRelaseItem;
	WJY_ResourceObjectDynamicState eDynamicState;
	intV_WJY iRefCount;
	ptrPItem = (WJY_ResourceSoundItem *)ptrItem;
	uintV_WJY uHash;
	ptrRelaseItem = NULL_WJY;
	if((ptrPItem == NULL_WJY) || (ptrPItem->hMgrAndle==NULL_WJY))
		return NULL_WJY;
	ptrMgr = (WJY_ResourceSoundManager *)(ptrPItem->hMgrAndle);
	uHash = ptrMgr->hashfunc(ptrPItem->ptrName,ptrPItem->iNSize,WJYRMgr_ConstID_RSoundMgrMapSlotMaxSize);
	WJYSysAPI_Syn_AcquireRWLockWriteWF(ptrMgr->List[uHash].lock);
		if(ptrItem->eDynamicState != WJYSys_Const_RODynamic_eReclaim){
			...
			if(iRefCount<=0){
				...
				if((eDynamicState != WJYSys_Const_RODynamic_eReclaim) && (WJYSysAPI_JudgeSetValueInt((volatile_WJY intV_WJY *)(&(ptrPItem->eDynamicState)),
						(intV_WJY)WJYSys_Const_RODynamic_eReclaim,
						(intV_WJY)eDynamicState)==true_WJY)){
					__native_Eg_RemoveMapItem<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,uHash,ptrPItem);
...
HWJY_Result Eg_DestorySoundMgrObject(HSoundMgrAndle hAndle,const_WJY char_WJY * ptrKey,intV_WJY iKeySize){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem * ptrItem;
	uintV_WJY uHash;
//	WJY_ResourceObjectDynamicState eDynamicState;
	...
	uHash = ptrMgr->hashfunc(ptrKey,iKeySize,WJYRMgr_ConstID_RSoundMgrMapSlotMaxSize);
	WJYSysAPI_Syn_AcquireRWLockWriteWF(ptrMgr->List[uHash].lock);
		ptrItem = __native_Eg_UnRefMapItem<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,uHash,ptrKey,iKeySize);
	WJYSysAPI_Syn_ReleaseRWLockWriteWF(ptrMgr->List[uHash].lock);
	if(ptrItem != NULL_WJY){
		__native_Eg_PushReclaimQueue<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,ptrItem);
	}
...
HWJY_Result Eg_PushSoundRTaskerQueue(WJY_ResourceSound * ptrItem){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem * ptrPItem;
	WJY_ResourceSoundItem * ptrReleaseItem;
	intV_WJY iRefCount;
	uintV_WJY uHash;
	WJY_ResourceObjectDynamicState eDynamicState;
	ptrPItem = (WJY_ResourceSoundItem *)ptrItem;
	...
	if(iRefCount<0){
		...
		if(ptrReleaseItem!=NULL_WJY){
			__native_Eg_PushReclaimQueue<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,ptrReleaseItem);
		}
		return HWJYResult_Fail;
	}
	if(HResultWJY_FAILED(WJYSysAPI_Syn_TryEnterSpinLock(&(ptrMgr->TkLock))) == true_WJY){
			...
			if(iRefCount == 0){
				WJYSysAPI_Syn_AcquireRWLockWriteWF(ptrMgr->List[uHash].lock);
					...
					if((eDynamicState != WJYSys_Const_RODynamic_eReclaim) && (WJYSysAPI_JudgeSetValueInt((volatile_WJY intV_WJY *)(&(ptrPItem->eDynamicState)),
							(intV_WJY)WJYSys_Const_RODynamic_eReclaim,
							(intV_WJY)eDynamicState)==true_WJY)){
						ptrReleaseItem = __native_Eg_RemoveMapItem<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,uHash,ptrPItem);
					}
				WJYSysAPI_Syn_ReleaseRWLockWriteWF(ptrMgr->List[uHash].lock);
				if(ptrReleaseItem!=NULL_WJY){
					__native_Eg_PushReclaimQueue<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr,ptrReleaseItem);
		...
		ptrPItem->ptrNext = (WJY_ResourceSoundItem *)(ptrMgr->ptrOverFlowQueueRoot);
		ptrMgr->ptrOverFlowQueueRoot = ptrPItem;
		WJYSysAPI_Syn_LeaveSpinLock(&(ptrMgr->TkOvLock));
		return HWJYResult_OK;
	}
	if(__native_Eg_PushPriorityStackSortList(ptrMgr,ptrPItem,WJYRMgr_ConstID_RSoundMgrTaskerListMaxL)==false_WJY){
...
HWJY_Result Eg_PushSoundRTaskerQueueFill(WJY_ResourceSound * ptrItem,WJY_CommResourceBlock * ptrBlock){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem * ptrPItem;
	WJY_ResourceSoundItem * ptrReleaseItem;
	intV_WJY iRefCount;
...
HWJY_Result Eg_PushSoundRTaskerQueueN(HSoundMgrAndle hAndle,const_WJY char_WJY * ptrKey,intV_WJY iKeySize){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem * ptrPItem;
	ptrMgr = (WJY_ResourceSoundManager *)hAndle;
	if(ptrMgr==NULL_WJY)
		return HWJYResult_Fail;
	ptrPItem = (WJY_ResourceSoundItem *)Eg_OpenPolygonMgrObject(hAndle,ptrKey,iKeySize);
	if(ptrPItem == NULL_WJY)
		return HWJYResult_Fail;
	if(WJYSysAPI_JudgeSetValueInt((volatile_WJY intV_WJY *)(&(ptrPItem->eState)),
			(intV_WJY)WJYSys_Const_ResLoad_eWJYBeginLoad,
			(intV_WJY)WJYSys_Const_ResLoad_eWJYUnLoadComplete)==false_WJY){
		Eg_ReleaseSoundMgrObject((WJY_ResourceSound *)ptrPItem);
		return HWJYResult_Fail;
	}
	if(HResultWJY_FAILED(WJYSysAPI_Syn_TryEnterSpinLock(&(ptrMgr->TkLock))) == true_WJY){
		if(HResultWJY_FAILED(WJYSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkOvLock))) == true_WJY){
...
HWJY_Result Eg_PushSoundRTaskerQueueNFill(HSoundMgrAndle hAndle,const_WJY char_WJY * ptrKey,intV_WJY iKeySize,WJY_CommResourceBlock * ptrBlock){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem * ptrItem;
	ptrMgr = (WJY_ResourceSoundManager *)hAndle;
	...
	ptrItem = (WJY_ResourceSoundItem *)Eg_OpenSoundMgrObject(hAndle,ptrKey,iKeySize);
	if(ptrItem == NULL_WJY)
		return HWJYResult_Fail;
	ptrBlock->ptrItem = ptrItem;
	if(HResultWJY_FAILED(WJYSysAPI_Syn_EnterSpinLock(&(ptrMgr->TBLock))) == true_WJY){
		Eg_ReleaseSoundMgrObject((WJY_ResourceSound *)ptrItem);
		return HWJYResult_Fail;
	}
	ptrBlock->ptrNext = ptrMgr->ptrRootCommBlock;
	ptrMgr->ptrRootCommBlock = ptrBlock;
...
HWJY_Result Eg_RefurbishSoundRBackgroundDispose(HSoundMgrAndle hAndle,WJY_ResourceBgDisposeParamer * ptrParamer){
	WJY_ResourceSoundManager * ptrMgr;
	WJY_ResourceSoundItem * ptrItem;
	WJY_ResourceSoundItem * ptrRelaseItem;
	WJY_ResourceObjectDynamicState eDynamicState;
	...
	if((ptrMgr == NULL_WJY) || (ptrParamer == NULL_WJY))
		return HWJYResult_Fail;
	ptrRelaseItem = NULL_WJY;
	if((ptrMgr->ptrOverFlowQueueRoot != NULL_WJY) && (ptrMgr->iTkCount<WJYRMgr_ConstID_RSoundMgrTaskerListMaxL)){
		ptrItem = NULL_WJY;
		if(HResultWJY_FAILED(WJYSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkOvLock))) == false_WJY){
			if(ptrMgr->ptrOverFlowQueueRoot!=NULL_WJY){
				ptrItem = (WJY_ResourceSoundItem *)(ptrMgr->ptrOverFlowQueueRoot);
		...
		if(ptrItem != NULL_WJY){
			if(HResultWJY_FAILED(WJYSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkLock))) == false_WJY){
				if(__native_Eg_PushPriorityStackSortList(ptrMgr,ptrItem,WJYRMgr_ConstID_RSoundMgrTaskerListMaxL)==false_WJY){
					if(HResultWJY_FAILED(WJYSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkOvLock))) == true_WJY){
						...
						ptrItem->ptrNext = (WJY_ResourceSoundItem *)(ptrMgr->ptrOverFlowQueueRoot);
	...
	if(ptrMgr->iTkCount>0){
		if(HResultWJY_FAILED(WJYSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkLock)))==false_WJY){
			ptrItem = (WJY_ResourceSoundItem *)(__native_Eg_PopMaxPriorityStackSort<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr));
			WJYSysAPI_Syn_LeaveSpinLock(&(ptrMgr->TkLock));
			if(ptrItem != NULL_WJY){
				...
				if(iRefCount==0){
					uHash = ptrMgr->hashfunc(ptrItem->ptrName,ptrItem->iNSize,WJYRMgr_ConstID_RSoundMgrMapSlotMaxSize);
					WJYSysAPI_Syn_AcquireRWLockWriteWF(ptrMgr->List[uHash].lock);
						eDynamicState = ptrItem->eDynamicState;
						if((eDynamicState != WJYSys_Const_RODynamic_eReclaim) && (WJYSysAPI_JudgeSetValueInt((volatile_WJY intV_WJY *)(&(ptrItem->eDynamicState)),
								(intV_WJY)WJYSys_Const_RODynamic_eReclaim,
	...
	if(ptrMgr->ptrReclaimQueueRoot != NULL_WJY){
		while((ptrItem = (WJY_ResourceSoundItem *)__native_Eg_PopReclaimQueue<WJY_ResourceSoundManager,WJY_ResourceSoundItem>(ptrMgr))!=NULL_WJY){
