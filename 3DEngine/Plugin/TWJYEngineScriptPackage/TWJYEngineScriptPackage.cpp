/*
 * TWJYEngineScriptPackage.cpp
 *
 *  Created on: 2009-8-14
 *      Author: Administrator
 */
#include "TWJYEngineScriptPackage.h"
...
//		WJYFileCFGHandle hCFGHandle;
		...
		ptrInface   = (WJY_ScriptLibraryHeavy *)WJYSysAPI_Mem_AllocateMemoryPool(iInfaceSize);
		if(ptrInface==NULL_WJY){

			return NULL_WJY;
		}
		WJYSysAPI_MemoryClearZero((void_WJY *)(ptrInface),iInfaceSize);
		iTableSize = WJY_CompilerAllocSize(WJY_ScriptLibraryFunTable) + sizeof(fun_WJY) * WJYScript_Table_FunMaxSize;
		ptrTable   = (WJY_ScriptLibraryFunTable *)WJYSysAPI_Mem_AllocateMemoryPool(iTableSize);
