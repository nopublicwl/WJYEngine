/*
 * TWJYEngineSysNetworkAPI_Linux.cpp
 *
 *  Created on: 2009-11-19
 *      Author: Administrator
 */
#include "../header/Linux/TWJYEngineSysNetworkAPI_Linux.h"
...
//			}
	...
	HSockCollection __WJYSysAPI_Net_CreateSockCollection(){
		WJY_SockCollection * ptrColl;

		ptrColl = (WJY_SockCollection *)WJYSysAPI_Mem_TLSAllocateMemoryPool(WJY_CompilerAllocSize(WJY_SockCollection));
		if(ptrColl == NULL_WJY)
			return NULL_WJY;
		WJYSysAPI_MemoryClearZero((void_WJY *)ptrColl,WJY_CompilerAllocSize(WJY_SockCollection));
		FD_ZERO(&(ptrColl->set));
		return (HSockCollection)ptrColl;
	}
	HWJY_Result __WJYSysAPI_Net_AddSocketToCollection(HSockCollection hColl,HWJYSocket hSK){
		WJY_SockCollection * ptrColl;

		ptrColl = (WJY_SockCollection *)hColl;
		if(ptrColl == NULL_WJY)
			return HWJYResult_Fail;
		if(hSK == Invalid_WJYSocket)
			return HWJYResult_Fail;
	...
	HWJY_Result __WJYSysAPI_Net_RemoveSocketForCollection(HSockCollection hColl,HWJYSocket hSK){
		WJY_SockCollection * ptrColl;
	...
	HWJY_Result __WJYSysAPI_Net_ClearSocketForCollection(HSockCollection hColl){
		WJY_SockCollection * ptrColl;
	...
	intV_WJY __WJYSysAPI_Net_GetSockCollectionCurrentSize(HSockCollection hColl){

		return 0;
	}
	intV_WJY __WJYSysAPI_Net_GetSockCollectionMaxSize(HSockCollection hColl){

		return 0;
	}
	void_WJY __WJYSysAPI_Net_DestroyCollection(HSockCollection hColl){
		WJY_SockCollection * ptrColl;
		ptrColl = (WJY_SockCollection *)hColl;
		if(ptrColl != NULL_WJY){
			FD_ZERO(&(ptrColl->set));
			WJYSysAPI_Mem_FreeMemoryPool((void_WJY *)ptrColl);
		}
	}
